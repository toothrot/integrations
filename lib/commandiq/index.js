...
function CommandIQ () {
  this.name = 'CommandIQ';
  this.baseUrl = 'http://test.commandiq.com/api/';
...
CommandIQ.prototype.identify = function (identify, settings, callback) {
  ...
  Object.keys(traits).forEach(function(t) {
    ...
    {
      // Send trait as identifUser param
...
CommandIQ.prototype.track = function (track, settings, callback) {
  ...
  Object.keys(properties).forEach(function(p) {
   if (validParam(properties[p]))
    {
      // Save trait as [key, value] to be sent as a trackEvent param.
      trackEventData.push([p, properties[p]]);
    }
  });
 ...
 */
function trackRequest(user_id, key, value, context, settings, callback) {
  var postData = {
    user_id: user_id,
    key: settings.apiKey,
    event_key: key,
  ...
  {
    case 'boolean':
      postData.event_type = 'bool';
      break;
    case 'string':
      postData.event_type = 'str';
      break;
    case 'number':
      postData.event_type = 'int';
      // Note: int_inc is not currently supported for segment.io.
      break;
    default:
