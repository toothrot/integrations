...
var debug       = require('debug')('segmentio:integrations:customer.io')
  ...
  , Identify    = require('segmentio-facade').Identify
...
CustomerIO.prototype.identify = function (identify, settings, callback) {
  var self = this;
  this._visit(identify, settings, function(err){
    ...
    request.put(req, self._handleResponse(callback));
  });
};
 ...
 * Group.
 *
 * this maps directly to identify, we send the group id as `.id`
 * and the rest of the traits we map as `Group Trait` : `value`.
 *
 * @param {Group} group
