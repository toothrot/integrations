...
HelpScout.prototype.validate = function (message, settings) {
  var err = this._validateSetting(settings, 'apiKey');
  if (err) return err;
...
function getTraits (identify) {
  ...
  if (identify.website()) traits.websites = [{ value : identify.website() }];
  if (identify.address()) traits.address  = [{ value : identify.address() }];
