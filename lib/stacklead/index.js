...
StackLead.prototype.validate = function(_, settings){
  ...
  if (!err && settings.delivery_method && VALID_DELIVERY_METHODS.indexOf(settings.delivery_method) === -1) {
    var deliveryMsg = fmt('"%s" integration requires delivery_method to be one of "%s" but was "%s"',
      this.name, VALID_DELIVERY_METHODS, settings.delivery_method);
    err = new integration.errors.Validation(deliveryMsg);
  }
  if (!err && settings.duplicates && settings.duplicates !== 'false') {
    var duplicateMsg = fmt('"%s" integration only permits "duplicates" setting to be "false" but was "%s"',
      this.name, settings.duplicates);
    err = new integration.errors.Validation(duplicateMsg);
  }
  return err;
};
...
StackLead.prototype.identify = function(payload, settings, fn){
  ...
  if (settings.duplicates) {
    payload.duplicates = settings.duplicates;
  }
  return this
    .post('')
