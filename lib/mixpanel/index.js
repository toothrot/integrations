...
Mixpanel.prototype.identify = function (identify, settings, callback) {
  ...
  var ignoreIp = identify.proxy('options.ignoreIp'); // TODO: move to mp opts
  ...
  var payload = {
    ...
    $ip          : ignoreIp ? identify.ip() : 0, // use the ip passed in options
    $ignore_time : identify.proxy('options.ignoreTime'), // TODO: move to mp opts
    mp_lib       : 'Segment.io'
  };
...
Mixpanel.prototype.track = function (track, settings, callback) {
  ...
  // Only add it to the query string if it exists
  if (settings.apiKey) req.qs.api_key = settings.apiKey;
