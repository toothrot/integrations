...
/**
 * First make sure that we initialize google analytics, then try and track
 * an event.
 */
...
GAClassic.prototype.track = function (track, settings, callback) {
  ...
  this._initialize(track, settings, function (err) {
    if (err) return callback(err);

    self._track(track, settings, callback);
  });
};


/**
 * We have to initialize Google Analytics with a pageview, otherwise the event
 * tracking doesn't seem to work :(
 */

GAClassic.prototype._initialize = function (track, settings, callback) {

  if (settings.initialized) return process.nextTick(callback);

  this._pageview(track, settings, callback);
};
...
GAClassic.prototype._track = function (track, settings, callback) {
